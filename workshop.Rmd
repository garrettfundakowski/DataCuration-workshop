...
## Intro to BioTIME and data curation

BioTIME is an open-access database for long-term assemblage data contributed from research groups globally. The power of our database is quite literally in the numbers. We currently have:
...
- Learn how to validate and check large datasets
- Use data wrangling to rectify any issues that come up in data checking
- Manipulate, aggregate, omit data records
...
#### RStudio repository project
4. And now we're ready to clone! Go to *File > New Project > Version Control*. 
...
```{r Setup, results='hide', messages=F}
...
Now let's use R to help us check the first criteria:
...
# For this, we can use:
# E.g. use: sort(unique(dt$plot)) and rename levels using:
# levels(dt$plot) <- plyr::revalue(levels(dt$plot), c("old_name"="new_name"))
...
- Check for duplication and misspellings in the species names  (e.g. *Puffinus spp. / Puffinus sp.*; unidentified/unknown; *Buenia jeffreysi / Buenia jeffreysii*; double spacing; several spaces, authors name & date, etc.). We're not validating or updating species.
...
**Uncertain or unidentified IDs**: "Genus sp" (without the full stop/period) or if the dataset distinguishes between different uncertain IDs, "Genus sp1" and "Genus sp2"
**No subspecies or varieties**: Lowest level = "Genus species"
**Species groups**: Some taxa are less resolved and are placed into aggregates. We denote this with "agg."
